...
def test_orderly_routing_initialization(
    orderly_vault: OrderlyVault,
    broker_id: str,
    orderly_account_id: str,
    arbitrum_sepolia_usdc: AssetIdentifier,
):
    """Test OrderlyRouting initialization with required parameters."""

    routing = OrderlyRouting(
        reserve_token_address=arbitrum_sepolia_usdc.address.lower(),
        vault=orderly_vault,
        broker_id=broker_id,
        orderly_account_id=orderly_account_id,
    )

    assert routing.vault == orderly_vault
    assert routing.broker_id == broker_id
    assert routing.orderly_account_id == orderly_account_id
    assert routing.reserve_token_address == arbitrum_sepolia_usdc.address
    assert routing.vault_interaction_gas_limit == 2_000_000


def test_orderly_routing_state_creation(
    orderly_vault: OrderlyVault,
    ...
    """Test creation of OrderlyRoutingState."""
    ...
    execution_details = {"tx_builder": orderly_tx_builder}

    routing_state = routing.create_routing_state(
        universe=orderly_strategy_universe,
        execution_details=execution_details
    )

    assert isinstance(routing_state, OrderlyRoutingState)
    assert routing_state.tx_builder == orderly_tx_builder
    assert routing_state.strategy_universe == orderly_strategy_universe
    assert routing_state.vault == orderly_vault
    assert routing_state.broker_id == broker_id
    assert routing_state.orderly_account_id == orderly_account_id

    # Test reserve asset retrieval
    reserve_asset = routing_state.get_reserve_asset()
    assert reserve_asset.address == arbitrum_sepolia_usdc.address


def test_orderly_routing_preflight_checks(
    orderly_vault: OrderlyVault,
    ...
    """Test preflight checks and logging."""
    ...
    routing.perform_preflight_checks_and_logging(orderly_pair_universe)
...
def test_orderly_routing_deposit_transaction_creation(
    ...
    orderly_strategy_universe: TradingStrategyUniverse,
    arbitrum_sepolia_usdc: AssetIdentifier,
    arbitrum_sepolia_weth: AssetIdentifier,
    broker_id: str,
    orderly_account_id: str,
):
    ...
    vault_pair = TradingPairIdentifier(
        base=arbitrum_sepolia_weth,  # Vault shares
        quote=arbitrum_sepolia_usdc,  # USDC denomination
    ...
    trade = TradeExecution(
        ...
        reserve_currency=arbitrum_sepolia_usdc,
...
def test_orderly_routing_setup_trades(
    ...
    with patch.object(orderly_routing_model, 'deposit_or_withdraw') as mock_deposit_withdraw:
        mock_deposit_withdraw.return_value = [Mock(), Mock()]  # Two transactions per trade
...
def test_orderly_routing_settle_trade_basic(
    ...
    with patch('tradeexecutor.ethereum.orderly.orderly_routing.get_swap_transactions') as mock_get_swap:
        mock_tx = Mock()
        mock_tx.tx_hash = "0x123"
        mock_get_swap.return_value = mock_tx

        # Mock get_block_timestamp
        with patch('tradeexecutor.ethereum.orderly.orderly_routing.get_block_timestamp') as mock_timestamp:
...
def test_orderly_routing_error_handling():
    ...
    state = Mock(spec=State)
    routing_state = Mock(spec=OrderlyRoutingState)
