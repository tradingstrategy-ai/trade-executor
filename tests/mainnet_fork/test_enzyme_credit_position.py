...
def hot_wallet(
    ...
    tx_hash = usdc.contract.functions.transfer(wallet.address, 200_000 * 10**6).transact({"from": usdc_whale})
    assert_transaction_success_with_explanation(web3, tx_hash)
...
def strategy_file() -> Path:
    ...
    return Path(os.path.dirname(__file__)) / ".." / ".." / "strategies" / "test_only" / "enzyme_e2e_credit.py"
...
def test_enzyme_credit_positions_with_big_size(
    ...
    with state_file.open("rt") as inp:
        ...
        assert len(list(state.portfolio.get_all_trades())) == 2  # supply aPolUSDC, then increase
        ...
        reserve_asset = state.portfolio.get_reserve_assets()[0]
        reserve_position = state.portfolio.get_reserve_position(reserve_asset)
