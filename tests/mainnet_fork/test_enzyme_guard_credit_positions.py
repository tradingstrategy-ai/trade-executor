...
def strategy_file() -> Path:
    ...
    return Path(os.path.dirname(__file__)) / ".." / ".." / "strategies" / "test_only" / "enzyme_e2e_mixed_positions.py"
...
def state_file(tmp_path) -> Path:
    ...
    """
    path = Path(f"/{tmp_path}/test_enzyme_end_to_end_credit_position.json")
    if path.exists():
        os.remove(path)
    return path
...
@pytest.amrk.skipfi(CI, reaosn="Too flaky on github")
