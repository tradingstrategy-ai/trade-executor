...
from tradeexecutor.state.identifier import IGNORE_REASON_LACKS_FEE_DATA, IGNORE_REASON_REQUIRES_WHITELIST
...
class _VaultPair:
    ...
    def __init__(self, name: str, address: str, permission: VaultDepositPermission, ignore_reason: str | None = None) -> None:
        self.base = SimpleNamespace(token_symbol=name)
        self.chain_id = ChainId.ethereum.value
        self.pool_address = address
        self.other_data = {}
        self._metadata = SimpleNamespace(deposit_permission=permission, protocol_slug="test-protocol")
