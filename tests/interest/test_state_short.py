...
def test_short_distribute_interest(
        state: State,
        weth_short_identifier: TradingPairIdentifier,
        usdc: AssetIdentifier,
):
    """Map accrued interest assets for shorts."""
    ...
    # Check that our interest distribution parameters look correct
    interest_distribution = prepare_interest_distribution(state.portfolio)
    vweth = weth_short_identifier.base
    ausdc = weth_short_identifier.quote
    assert interest_distribution.assets == {vweth, ausdc}
    assert len(interest_distribution.entries) == 2
    assert interest_distribution.totals[vweth] == pytest.approx(Decimal('0.53333333'))
    assert interest_distribution.totals[ausdc] == pytest.approx(Decimal('1800') )

    entry_1 = interest_distribution.entries[ausdc]
    assert entry_1.asset == ausdc
    assert entry_1.weight == 1

    entry_2 = interest_distribution.entries[vweth]
    assert entry_2.asset == vweth
    assert entry_2.weight == 1
