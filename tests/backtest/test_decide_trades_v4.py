...
def test_decide_trades_v04(strategy_universe, tmp_path):
    ...
    # Changes everey pandas_ta release
    # assert indicator_cache_keys[1].get_cache_key() == "rsi_cddc4a87(length=21)-WETH-USDC"
    # assert indicator_cache_keys[0].get_cache_key() == "bb_3bdd79e8(length=20)-WETH-USDC"

    assert indicator_cache_keys[1].get_cache_key().startswith("rsi_") # cddc4a87(length=21)-WETH-USDC"
    assert indicator_cache_keys[0].get_cache_key().startswith("bb_") # 3bdd79e8(length=20)-WETH-USDC"
...
def test_decide_trades_v04_cached(strategy_universe, tmp_path):
    """Test DecideTradesProtocolV4 with cache indicator data

    - Run the backtest twice and see the results are cached
    """
    ...
    assert len(result_1.state.portfolio.closed_positions) == 15
    assert len(result_2.state.portfolio.closed_positions) == 15

    indicators = result_2.indicators
    ...
    bb_result = indicators.indicator_results[indicator_cache_keys[1]]
    assert bb_result.universe_key == "polygon_1d_WETH-USDC_2023-01-01-2023-02-01"
    assert bb_result.cached is True
    assert bb_result.data is not None
