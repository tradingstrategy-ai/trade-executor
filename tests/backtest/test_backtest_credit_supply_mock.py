...
def test_backtest_open_and_close_credit_supply_mock_data(
    ...
    usdc = universe.get_single_pair().quote
    assert interest_events[0].asset.get_pricing_asset() == usdc

    assert credit_position.calculate_accrued_interest_quantity(usdc) == Decimal('14')  # Non-denormalised interest
