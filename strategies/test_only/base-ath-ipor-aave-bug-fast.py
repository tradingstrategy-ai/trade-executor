"""Lightweight variant of base-ath-ipor-aave-bug.py for fast unit testing.

Uses explicit pair list with load_partial_data() instead of the full
Base chain scan (fetch_pair_universe + fetch_tvl + TokenSniffer filtering).
All strategy logic (decide_trades, indicators, yield rules) is inherited
from the original module unchanged.
"""
import datetime
import logging
...
def create_trading_universe(
    ...
    """Lightweight universe construction using explicit pair list.

    Replaces the full chain scan with direct load_partial_data() call,
    skipping fetch_pair_universe, fetch_tvl, load_token_metadata, and
...
# ---------------------------------------------------------------------------
# Everything below is identical to base-ath-ipor-aave-bug.py
# (decide_trades, indicators, yield rules)
# ---------------------------------------------------------------------------
