...
def decide_trades(
    ...
    for pair_id in volume_inclusion_criteria_pairs:
        ...
        if volatility < btc_vol:
            # Volatility must be higher than in BTC
            continue

        if volatility > max_vol[0]:
            max_vol = (volatility, pair)

        # Will be normalised below alpha_model.normalise_weights()
        weight = 1 / volatility
...
- The Index is comprised of liquid memecoins listed on Ethereum mainnet.
