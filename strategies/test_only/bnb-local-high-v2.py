...
def create_trading_universe(
    ...
    # Perform web3 checks
    if execution_context.live_trading:
        compat_db = check_tokens_for_lagoon(
            strategy_universe,
            lagoon_execution=cast(LagoonExecution, input.execution_model),
            max_tokens=3,
        )

        stats = compat_db.calculate_stats()
        debug_printer(f"Compatibility database has: {compat_db.get_count()} entries\n{pformat(stats)}")
