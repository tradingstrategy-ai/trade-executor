...
- Revalidate the cached vault price history parquet against the remote file on every live universe load. The local cache was trusted for 24 hours with no HEAD request, so a live executor could start on a parquet published before the last check without any signal that it had fallen behind. Live modes now pass `revalidate=True` through `load_partial_data()`; an unchanged remote still short-circuits the download, so this costs one HEAD request rather than a ~200 MB re-download. Backtests keep the plain cached path (2026-07-27)
