...
- Vault metadata and vault share price history now download through `VaultDataClient` and the Creem vault dataset API, which needs its own `VAULT_PRO_ACCESS_KEY`. The old unauthenticated `top-defi-vaults.tradingstrategy.ai` and `vault-protocol-metadata.tradingstrategy.ai` buckets are no longer served, so a live strategy loading vault data cannot start without the new key (2026-09-01)
