...
- Fix multichain Lagoon cross-chain (CCTP) `trade-ui` test trades crashing with `UnboundLocalError: cannot access local variable 'failed_tx'` during the satellite-chain buy: the sequential/MEV-blocker broadcast path's buy-side treasury trip-wire compared the buy value against the home-chain reserve balance, which is legitimately zero after the reserve was bridged to the satellite chain via CCTP, so it tripped and then crashed referencing an unset `failed_tx` (masking the real error). The trip-wire now skips cross-chain (satellite) trades whose pair lives on a different chain than the execution model, and always initialises `failed_tx` so a genuine same-chain underfunded buy raises a clean `ExecutionHaltableIssue` (2026-06-11)
