...
- **Chain-aware YieldManager.** The hub-chain venue precondition is documented
  ...
  *Partial mitigation (issue #1562).* Idle capital that settles on a satellite
  ...
  execution batch when it goes `cctp_in_transit` and resolves via
  restart/retry, so the sweep makes that operational path routine — the startup
  ordering (CCTP retry before treasury sync and the accounting check in
  `cli/loop.py`) is load-bearing for correct reserve accounting.
