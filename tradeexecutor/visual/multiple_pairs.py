...
def export_trades_as_dataframe_by_trading_pair(portfolio: Portfolio, pair: TradingPairIdentifier):
    """Convert executed trades for a specific pair to a dataframe, so it is easier to work with them in Plotly."""
    data = [export_trade_for_dataframe(portfolio, t) for t in portfolio.get_all_trades_by_trading_pair(pair)]
    return pd.DataFrame(data)
...
def visualise_multiple_pairs(
    ...
    for i in range(len(pair_ids)):
        ...
        if not end_at:
            end_at = candle_universe_single.get_timestamp_range()[1]
        ...
        trades_df = export_trades_as_dataframe_by_trading_pair(state.portfolio, identifier)
