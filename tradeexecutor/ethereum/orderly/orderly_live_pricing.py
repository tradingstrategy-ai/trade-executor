...
class OrderlyPricing(PricingModel):
    ...
    def get_usd_tvl(
        ...
        """Get the TVL of an Orderly vault pair."""
        assert pair.quote.is_stablecoin(), f"Only stablecoin vaults are supported for TVL, got: {pair}"

        block_number = self.web3.eth.block_number

        # TODO: Implement proper TVL fetching from Orderly vault
        # This would require calling the vault contract to get total assets
        # For now, return 0 as placeholder
        logger.warning(f"TVL fetching not yet implemented for Orderly vault {pair}")
        return 0.0
