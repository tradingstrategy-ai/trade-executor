...
class PandasTraderRunner(StrategyRunner):
    ...
    def on_clock(
        ...
        assert state.sync.treasury.last_updated_at is not None, "Cannot do trades before treasury is synced at least once. Run trade-executor correct-accounts to fix."
        ...
        if universe.is_single_pair_universe():
            ...
            if small_image is not None:
                if isinstance(small_image, bytes):
                    ...
                    logger.info("Chart visualisation missing")
            else:
