...
class TradingStrategyUniverse(StrategyExecutionUniverse):
    ...
    def get_pair_by_vault_name(self, name: str) -> TradingPairIdentifier:
        ...
        pair = translate_trading_pair(matches[0], cache=self.pair_cache)
        assert pair.is_vault(), f"Found pair is not a vault: {pair}"
        return pair
    ...
    def construct_universe(
        ...
        with self.execution_context.timed_task_context_manager(task_name="create_trading_universe"):
            universe = call_create_trading_universe(
                ...
                create_trading_universe=self.create_trading_universe,
                timestamp=ts,
                execution_model=execution_model,
                strategy_parameters=strategy_parameters,
